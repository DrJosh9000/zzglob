...
func TestGlob_SymlinkDirFullMatch(t *testing.T) {
	...
	pattern := filepath.ToSlash(filepath.Join("fixtures", "root", "**", "*"))
	p, err := Parse(pattern)
	if err != nil {
		t.Fatalf("Parse(%q) = %v", pattern, err)
	}
	...
	want := walkFuncCalls{
		calls: []walkFuncArgs{
			...
			{Path: "fixtures/root/a/b/link/c/result.txt"}, // <--- this one in particular
			{Path: "fixtures/root/a/b/real_dir"},
			{Path: "fixtures/root/a/b/real_dir/c"},
			{Path: "fixtures/root/a/b/real_dir/c/result.txt"},
		},
	}
